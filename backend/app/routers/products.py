...
from ..utils.validation import validate_file_size, validate_image_content, sanitize_string
import json
...
def list_produtos(
    ...
    if q:
        q_sanitized = sanitize_string(q, max_length=100)
        like = f"%{q_sanitized}%"
        query = query.filter(
...
def create_produto(
    ...
    produto_data = payload.model_dump()
    produto_data["nome"] = sanitize_string(produto_data["nome"], max_length=150)
    if produto_data.get("descricao"):
        produto_data["descricao"] = sanitize_string(produto_data["descricao"], max_length=5000)
    if produto_data.get("categoria"):
        produto_data["categoria"] = sanitize_string(produto_data["categoria"], max_length=100)
...
def update_produto(
    ...
    if "nome" in update_data:
        update_data["nome"] = sanitize_string(update_data["nome"], max_length=150)
    if "descricao" in update_data and update_data["descricao"]:
        update_data["descricao"] = sanitize_string(update_data["descricao"], max_length=5000)
    if "categoria" in update_data and update_data["categoria"]:
        update_data["categoria"] = sanitize_string(update_data["categoria"], max_length=100)
...
async def upload_imagem_produto(
    ...
    validate_file_size(len(conteudo))
    
    validate_image_content(conteudo, arquivo.content_type)
    
    filename_sanitized = sanitize_string(arquivo.filename or "image", max_length=255)
    
    up = upload_image(produto_id, conteudo, filename_sanitized, arquivo.content_type)
